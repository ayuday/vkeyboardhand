 ...
 * 纯 HTML + JS 实现，零依赖，兼容 Vue / React / Angular 等前端框架，
 * 支持本地引入、CDN 引用、npm 安装。
 *
 * 核心能力：
 *  - 加载 / 内联注入 keyboard.svg 与 hand.svg（fetch 或字符串）
 *  - KeyboardEvent 监听真实按键，键盘高亮 + 手势图联动
 *  - 点击键盘键位也可触发演示（无需真实键盘）
 *  - 键位 → 手指 → SVG id 三层映射（可自定义覆盖）
 *  - 指法标签、手指配色、主题切换、回调事件、完整生命周期 API
 *
 * GitHub : https://github.com/ayuday/vkeyboardhand
 * Issues : https://github.com/ayuday/vkeyboardhand/issues
 * npm    : https://www.npmjs.com/package/vkeyboardhand
 * MIT License
 */
